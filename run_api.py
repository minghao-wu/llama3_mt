...
def send_request(system_prompt, prompt, model_id):
...
def find_last_double_quote(char_list):
...
def fix_json_string(s):
    quotes_positions = [
        ...
        find_kth_double_quote(s, 3),
        find_last_double_quote(s)
    ]
    s = list(s)
...
def get_response(system_prompt, prompt, model_id):
    ...
    while True:
        try:
            ...
            translation = json.loads(fixed_output)["translation"].replace("####", '"')
            break
...
for k, v in language_map.items():
    ...
    system_prompt = 'You are an accurate translator. You MUST answer with the following JSON scheme: {"translation": string}'
    results = []
    for src, ref, prompt in tqdm(list(zip(src_sents, tgt_sents, prompts))):
        translation, generated_text = get_response(system_prompt, prompt, model_id)
        dic = {
            "id": str(uuid.uuid4()),
            "model_id": model_id,
            "src_lang": src_lang,
